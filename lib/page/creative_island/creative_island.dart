...
class _CreativeIslandCreatePageState extends State<CreativeIslandCreatePage>
  ...
  /// Build the creative island edit area
...
/// Build the text generation toolbar
...
void onGenerate(
 ...
 final wordCount = int.parse(
  ...
  // Image to Image, upload the image first
    ...
    params = <String, dynamic>{
      ...
      'ai_rewrite': _enableAIRewrite,
      'image':
          'https://${selectedImagePath ?? 'demo'}', // Used for testing consumption only, will be replaced with a URL after uploading
    };
  }
  ...
  request() async {
    try {
      ...
      Navigator.push(
        ...
        context
            .read<CreativeIslandBloc>()
            .add(CreativeIslandHistoriesLoadEvent(widget.id, forceRefresh: true));
      });
    } catch (e) {
      stopPeriodQuery = true;
      cancel();
  ...
  try {
    ...
    if (res.cost > 0) {
      ...
      openConfirmDialog(
        context,
        '【Test Use Only】\nThis request is expected to consume ${res.cost} Wisdom Fruits, do you want to continue?',
        () => request(),
      );
    } else {
      request();
